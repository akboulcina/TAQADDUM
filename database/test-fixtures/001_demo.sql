...
BEGIN
  ...
  IF v_tenant_id IS NULL THEN
    ...
    VALUES (
      v_tenant_id, 'demo-tenant', 'Démo', 'تجريبي', 'Demo',
      'active', gen_random_uuid(), gen_random_uuid()
    );
  ELSE
    UPDATE org.tenants
    SET name_fr = 'Démo',
  ...
  IF v_org_id IS NULL THEN
    ...
    INSERT INTO org.organizations (
      id, tenant_id, organization_code,
      name_fr, name_ar, name_en, status,
      created_by, updated_by
    )
    VALUES (
      v_org_id, v_tenant_id, 'demo-org',
      'Démo Org', 'منظمة تجريبية', 'Demo Org', 'active',
      gen_random_uuid(), gen_random_uuid()
    );
  ELSE
    UPDATE org.organizations
    SET name_fr = 'Démo Org',
  ...
  IF v_actor_id IS NULL THEN
    ...
    VALUES (
      gen_random_uuid(), v_tenant_id, 'admin@example.test', 'Demo Admin',
      'active', gen_random_uuid(), gen_random_uuid()
    );
  ELSE
